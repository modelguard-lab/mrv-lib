...
class TestMonitoring:
    ...
    def test_check_alerts_below_threshold(self):
        ...
        new_rows = pd.DataFrame([{
            ...
            "mean_ari": 0.2, "mean_ari_7d_avg": None,
            "delta_vs_baseline": None, "alert_fired": False,
        }])
        alerts = _check_alerts(
            new_rows,
            pd.DataFrame(columns=["date", "asset", "validator", "mean_ari"]),
            {"alert_ari_below": 0.3})
        assert len(alerts) == 1
    ...
    def test_check_alerts_no_alert(self):
        ...
        new_rows = pd.DataFrame([{
            ...
            "mean_ari": 0.8, "mean_ari_7d_avg": None,
            "delta_vs_baseline": None, "alert_fired": False,
        }])
        assert len(_check_alerts(
            new_rows,
            pd.DataFrame(columns=["date", "asset", "validator", "mean_ari"]),
    ...
    def test_idempotency(self):
        ...
        history = pd.DataFrame([{
            "date": "2026-03-23", "asset": "SPY", "validator": "rep", "mean_ari": 0.5,
        }])
