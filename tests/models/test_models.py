...
class TestModels:
    ...
    def test_fit_gmm_insufficient_data(self):
        from mrv.models.gmm import fit_gmm
        assert fit_gmm(pd.DataFrame(np.random.randn(5, 2), columns=["a", "b"]), K=3) is None
    ...
    def test_fit_hmm_insufficient_data(self):
        from mrv.models.hmm import fit_hmm
        assert fit_hmm(pd.DataFrame(np.random.randn(5, 2), columns=["a", "b"]), K=3) is None
    ...
    def test_register_custom_model(self):
        ...
        register_model("dummy", lambda features, K=2, **kw: np.zeros(len(features), dtype=int))
        labels = fit(
            pd.DataFrame(np.random.randn(50, 2), columns=["a", "b"]),
            model="dummy", n_states=2,
        )
        assert (labels == 0).all()
