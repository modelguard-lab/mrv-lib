...
class TestFactors:
    ...
    def test_var_cvar(self):
        from mrv.data.factors import cvar, log_returns, var
    ...
    def test_stability(self):
        from mrv.data.factors import log_returns, stability, volatility
        p = make_price_series(200)
        r = log_returns(p)
        v = volatility(r, window=20)
    ...
    def test_build_factors_default(self):
        from mrv.data.factors import DEFAULT_FACTORS, build_factors
        p = make_price_series(300)
        df = build_factors(p)
