...
from mrv.utils.config import _normalize_freq, get_assets
...
def _fetch_bars(
    ...
    is_forex = (getattr(contract, "secType", None) == "CASH"
                or contract.__class__.__name__ == "Forex")
    is_contfut = (getattr(contract, "secType", None) == "CONTFUT"
                  or contract.__class__.__name__ == "ContFuture")
    ...
    if is_contfut:
        ...
        while current_end > start_ts.to_pydatetime():
            ...
            for b in bars:
                t = b.date
                ts = (pd.Timestamp(datetime.fromtimestamp(t.timestamp()))
                      if hasattr(t, "timestamp") else pd.Timestamp(t))
                ts = ts.tz_localize(tz) if ts.tzinfo is None else ts.tz_convert(tz)
                bar_times.append(ts)
...
def download(cfg: Optional[Dict[str, Any]] = None) -> None:
    ...
    try:
        ...
        logger.error(
            "IB connect failed: %s (host=%s port=%s). Is Gateway/TWS running?",
            e, host, port,
        )
        raise
